...
class ShackHartmann:
    ...
    def construct_subapertures_fresnel(
        ...
        # Frequency filtering
        idxs, _ = self.filter.frequency_symmetric_filtering(
            time_window, fs, f0, high_freq=f1
        )
    ...
    def construct_subapertures_angular(
        ...
        freq_idxs, _ = self.filter.frequency_symmetric_filtering(
            time_window, fs, f0, high_freq=f1
        )
        ...
        U_fft = self.prop.bm.fft.fft2(U0, axes=(-2, -1)) * self.prop.bm.fft.fftshift(
            H, axes=(-2, -1)
        )
    ...
    def calculate_displacements(
        ...
        xcorr = xp.abs(
            xp.fft.fftshift(
                xp.fft.ifft2(cp_corr / (xp.abs(cp_corr) + 1e-12)), axes=(1, 2)
            )
        )
    ...
    def calculate_displacements_graph_laplacian(
        ...
        for i in (
            valid_ids.tolist() if hasattr(valid_ids, "tolist") else list(valid_ids)
        ):
            i = int(i)
            cp_corr = F * F[i : i + 1].conj()
            cp_corr /= xp.abs(cp_corr) + eps
        ...
        Sy_up = xp.where(
            both,
            0.5 * (S_y - S_y.T),
            xp.where(only_ij, S_y, xp.where(only_ji, -S_y.T, 0)),
        )
        Sx_up = xp.where(
            both,
            0.5 * (S_x - S_x.T),
            xp.where(only_ij, S_x, xp.where(only_ji, -S_x.T, 0)),
        )
