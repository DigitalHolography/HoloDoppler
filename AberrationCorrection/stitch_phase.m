function phase = stitch_phase(shifts, phase_zernike, numX, numY, shack_hartmann)

SubAp_numX = floor(numX / shack_hartmann.n_SubAp);
SubAp_numY = floor(numY / shack_hartmann.n_SubAp);

%% Padding when n_subAp_positions > image_to_subap_size_ratio
stride = floor((numX - SubAp_numX) / (shack_hartmann.n_SubAp_inter - 1));

if stride > SubAp_numX
    range_x = stride * (shack_hartmann.n_SubAp_inter - 1) + SubAp_numX;
    range_y = stride * (shack_hartmann.n_SubAp_inter - 1) + SubAp_numY;
else
    range_x = stride * (shack_hartmann.n_SubAp_inter);
    range_y = stride * (shack_hartmann.n_SubAp_inter);
end

% pad_x = floor(SubAp_numX/2) - floor(stride/2);
% pad_y = floor(SubAp_numX/2) - floor(stride/2);
pad_x = floor((numX - range_x) / 2);
pad_y = floor((numY - range_y) / 2);
phase = zeros(numX, numY, size(shifts, 3));

shifts = double(shifts);

if ~(isempty(phase_zernike))
    %     shifts = shifts .* numX .* 3.1427;
    shifts = shifts .* (numX / shack_hartmann.n_SubAp);
end

% shifts = shifts .* 3;
% shifts = reshape(shifts, shack_hartmann.n_SubAp_inter, shack_hartmann.n_SubAp_inter, []);

% a = DopplerAcquisition(numX, numX, 0,0,0,0,0,0,0,0,0);
% [~, defocus] = zernike_phase(9, numX, numX);
...
% sh = zeros(50, 1);
[~, tilt] = zernike_phase(2, numX, numY);
cf = linspace(-60, 60, 50);
sh = zeros(50, 1);
half_SubAp_numX = floor(SubAp_numX / 2);
% half_SubAp_numX = floor(numX /2) - 1;
half_numX = floor(numX / 2);

for i = 1:50
    calibration_factor = cf(i);
    tilt_phase_pos = exp(1i .* (tilt));
    tilt_phase_pos = tilt_phase_pos(half_numX - half_SubAp_numX:half_numX + half_SubAp_numX, half_numX - half_SubAp_numX:half_numX + half_SubAp_numX);
    ...
    %     tilt_phase_neg = tilt_phase_neg(half_numX - half_SubAp_numX : half_numX + half_SubAp_numX, half_numX - half_SubAp_numX : half_numX + half_SubAp_numX);
...
fit_coef = f.p1;
% disp(fit_coef* (numX/shack_hartmann.n_SubAp))
% disp(fit_coef)
% % %
...
for i = 1:size(shifts, 3)
    ...
    if ~(isempty(phase_zernike))
        A = A - (A(half_numX, half_numX) - phase_zernike(half_numX, half_numX));
    else
        A = A - mean(A, "all");
    end
...
mask = construct_mask(0, round(numX / 2), numX, numY);
