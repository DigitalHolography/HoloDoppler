classdef AberrationCorrection
    ...
    methods
        ...
        function [rephasing_zernikes, shack_zernikes, iterative_opt_zernikes] = generate_zernikes(obj, numX, numY)

            if ~isempty(obj.rephasing_zernike_indices)
                % [~, rephasing_zernikes] = zernike_phase([2 1], numX, numY);
                rephasing_zernikes = evaluate_zernikes([1 1], [1 -1], numX, numY);
            else
                rephasing_zernikes = [];
            end

            if ~isempty(obj.shack_hartmann_zernike_indices)
                [~, shack_zernikes] = zernike_phase(obj.shack_hartmann_zernike_indices, numX, numY);
            else
                shack_zernikes = [];
            end

            if ~isempty(obj.iterative_opt_zernike_indices)
                [~, iterative_opt_zernikes] = zernike_phase(obj.iterative_opt_zernike_indices, numX, numY);
            else
