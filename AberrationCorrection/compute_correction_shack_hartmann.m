function [correction, stiched_moments_video, shifts_vector, stitched_correlation_video] = compute_correction_shack_hartmann(istream, cache, kernel, rephasing_data, gw, complex_mask, ...
    ...
    acquisition = DopplerAcquisition(numX, numY, cache.Fs / 1000, cache.z, cache.z_retina, cache.z_iris, cache.wavelength, cache.DX, cache.DY, cache.pix_width, cache.pix_height);
    ...
    stiched_moments_video = zeros(numX, numY, 1, num_batches, 'single');
    shifts_vector = zeros(num_subapertures_inter ^ 2, num_batches);
    stitched_correlation_video = zeros(numX, numY, 1, num_batches, 'single');
    ...
    parfor (batch_idx = 1:num_batches, parfor_arg)
        ...
        if nin == 23 % change this value if function arguments are added or removed
            ...
            [~, previous_zernikes] = zernike_phase(previous_p, numX, numY);
            previous_phase = 0;
        ...
        stitched_correlation_video(:, :, :, batch_idx) = imresize(stiched_corr_subapp, [numX numY]);
