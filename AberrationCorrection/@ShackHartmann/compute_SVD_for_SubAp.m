...
for SubAp_idy = SubAp_idRange
    ...
    for SubAp_idx = SubAp_idRange
        ...
        %             idxRange_out = (SubAp_idx-1)*floor(ac.Nx/obj.n_SubAp)+SubAp_init:(SubAp_idx-1)*floor(ac.Nx/obj.n_SubAp)+SubAp_init+numel(SubAp_init:SubAp_end)-1;
        %             idyRange_out = (SubAp_idy-1)*floor(ac.Ny/obj.n_SubAp)+SubAp_init:(SubAp_idy-1)*floor(ac.Ny/obj.n_SubAp)+SubAp_init+numel(SubAp_init:SubAp_end)-1;
        moment_chunks_crop_array(idyRange, idxRange) = moment_chunk_cropped;
        %             c = imgaussfilt(c, ceil(min(size(c, 1), size(c, 2))/10));
        %             c = imbinarize(c,'adaptive', 'ForegroundPolarity', 'bright', 'Sensitivity', 0.5);
        %             FH_reduced((SubAp_idy - 1) * obj.n_SubAp + SubAp_idx, :, :) = FH(idyRange,idxRange,:);
...
central = stitched_FH_reduced(idyRange_ref, idxRange_ref);
...
central_bis = stitched_FH_reduced_bis(idyRange_ref, idxRange_ref);
