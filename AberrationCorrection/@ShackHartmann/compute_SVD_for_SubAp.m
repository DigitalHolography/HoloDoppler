...
SubAp_numX = floor(ac.numX / obj.n_SubAp);
SubAp_numY = floor(ac.numY / obj.n_SubAp);
...
if ~calibration
    ...
    M0 = imresize(M0, [floor(ac.numY / obj.n_SubAp) floor(ac.numX / obj.n_SubAp)]);
end

mask = ones(SubAp_numY, SubAp_numX);
mask(1:obj.SubAp_margin * SubAp_numY, :) = 0;
mask(:, 1:obj.SubAp_margin * SubAp_numX) = 0;
mask(:, end - obj.SubAp_margin * SubAp_numX:SubAp_numY) = 0;
mask(end - obj.SubAp_margin * SubAp_numX:SubAp_numX, :) = 0;
mask = imgaussfilt(mask, 10);

SubAp_init = max(1, floor(obj.SubAp_margin * floor(double(ac.numX) / obj.n_SubAp)));
SubAp_end = ceil(ac.numX / obj.n_SubAp - SubAp_init);
...
moment_chunk_mask = ones(floor(double(ac.numX) / obj.n_SubAp));

%     FH_reduced = ones((obj.n_SubAp)^2, size(moment_chunk_mask,1) + floor(ac.numY/obj.n_SubAp) - 1, size(moment_chunk_mask,1) + floor(ac.numY/obj.n_SubAp) - 1);
FH_reduced = ones((obj.n_SubAp) ^ 2, floor(ac.numY / obj.n_SubAp), floor(ac.numY / obj.n_SubAp), size(FH, 3));
FH_reduced_bis = zeros(size(FH_reduced));
%     FH_reduced_bis = ones((obj.n_SubAp)^2, size(moment_chunk_mask,1) + floor(ac.numY/obj.n_SubAp) - 1, size(moment_chunk_mask,1) + floor(ac.numY/obj.n_SubAp) - 1);
stitched_FH_reduced = ones((obj.n_SubAp) * (size(moment_chunk_mask, 1) + floor(ac.numY / obj.n_SubAp) - 1), (obj.n_SubAp) * (size(moment_chunk_mask, 1) + floor(ac.numY / obj.n_SubAp) - 1));
stitched_FH_reduced_bis = ones((obj.n_SubAp) * (size(moment_chunk_mask, 1) + floor(ac.numY / obj.n_SubAp) - 1), (obj.n_SubAp) * (size(moment_chunk_mask, 1) + floor(ac.numY / obj.n_SubAp) - 1));
...
moment_chunks_crop_array = zeros(ac.numX, ac.numY); %Stitched cropped PowerDoppler moments in each subaperture
...
for SubAp_idy = SubAp_id_range
    ...
    for SubAp_idx = SubAp_id_range
        ...
        %             idx_range_out = (SubAp_idx-1)*floor(ac.numX/obj.n_SubAp)+SubAp_init:(SubAp_idx-1)*floor(ac.numX/obj.n_SubAp)+SubAp_init+numel(SubAp_init:SubAp_end)-1;
        %             idy_range_out = (SubAp_idy-1)*floor(ac.numY/obj.n_SubAp)+SubAp_init:(SubAp_idy-1)*floor(ac.numY/obj.n_SubAp)+SubAp_init+numel(SubAp_init:SubAp_end)-1;
