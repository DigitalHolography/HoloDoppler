...
% SubAp_idx < SubAp_idx
% SubAp_idy < SubAp_idy
% SubAp_numX < n_SubAp Attention : numX = nb de pixels et non nb de subAp
% SubAp_numY
...
%(SubAp_idx-1)*floor(ac.numX/obj.n_SubAp) + 1:SubAp_idx*floor(ac.numX/obj.n_SubAp), (SubAp_idy-1)*floor(ac.numY/obj.n_SubAp) + 1:SubAp_idy*floor(ac.numY/obj.n_SubAp)
...
%(mid_SubAp-1)*floor(ac.numX/obj.n_SubAp) + 1:mid_SubAp*floor(ac.numX/obj.n_SubAp),...
%(mid_SubAp-1)*floor(ac.numY/obj.n_SubAp) + 1:mid_SubAp*floor(ac.numY/obj.n_SubAp)
...
%         [~,phi] = zernike_phase(obj.modes(p), numX, numY);
% %         rand_coef(p) = rand;
...
numXX = floor(ac.numX / obj.n_SubAp); % size of new SubAp in x
numYY = floor(ac.numX / obj.n_SubAp); % size of new SubAp in y
...
if ~calibration
    ...
    M0 = imresize(M0, [ac.numY / obj.n_SubAp ac.numX / obj.n_SubAp]);
else
    x = linspace(-1, 1, numXX);
    y = linspace(-1, 1, numXX);
    [X, Y] = meshgrid(x, y);
    A = 1;
    sigma = 1 / numXX * obj.n_SubAp;
    G = A * exp(-1 / (sigma ^ 2) * ((Y) .^ 2 + (X) .^ 2));
    G = imresize(G, [numXX numXX]);
end
...
SubAp_init = max(1, floor(obj.SubAp_margin * floor(double(resize_ratio * ac.numX) / obj.n_SubAp)));
% SubAp_init = max(1,floor(double(ac.numX)/obj.n_SubAp));
SubAp_end = ceil(resize_ratio * ac.numX / obj.n_SubAp - SubAp_init);
...
moment_chunks_array = zeros(ac.numX, ac.numY); %Stitched PowerDoppler moments in each subaperture
moment_chunks_crop_array = zeros(resize_ratio * ac.numX, resize_ratio * ac.numY); %Stitched cropped PowerDoppler moments in each subaperture
SubAp_id_range = [SubAp_idref:obj.n_SubAp 1:SubAp_idref - 1];
%     correlation_chunks_array = zeros((SubAp_end-SubAp_init+floor(ac.numX/obj.n_SubAp))*obj.n_SubAp); %Stitched cropped correlations in each subaperture
correlation_chunks_array = zeros((SubAp_end - SubAp_init + resize_ratio * floor(ac.numX / obj.n_SubAp)) * obj.n_SubAp_inter); %Stitched cropped correlations in each subaperture

%     gw = (100 * (ac.numX/obj.n_SubAp)/512)/obj.n_SubAp;
gw = 20;
%     gw = 50;
gw = gw / obj.n_SubAp;
...
%             idx_range = (SubAp_idx-1)*floor(ac.numX/obj.n_SubAp)+1:SubAp_idx*floor(ac.numX/obj.n_SubAp);
%             idy_range = (SubAp_idy-1)*floor(ac.numY/obj.n_SubAp)+1:SubAp_idy*floor(ac.numY/obj.n_SubAp);
%             idx_range_ref = (SubAp_idref-1)*floor(ac.numX/obj.n_SubAp)+1:SubAp_idref*floor(ac.numX/obj.n_SubAp);
%             idy_range_ref = (SubAp_idref-1)*floor(ac.numY/obj.n_SubAp)+1:SubAp_idref*floor(ac.numY/obj.n_SubAp);
...
%             idx_range_out = (SubAp_idx-1)*floor(resize_ratio*ac.numX/obj.n_SubAp)+SubAp_init:(SubAp_idx-1)*floor(resize_ratio*ac.numX/obj.n_SubAp)+SubAp_init+numel(SubAp_init:SubAp_end)-1;
%             idy_range_out = (SubAp_idy-1)*floor(resize_ratio*ac.numY/obj.n_SubAp)+SubAp_init:(SubAp_idy-1)*floor(resize_ratio*ac.numY/obj.n_SubAp)+SubAp_init+numel(SubAp_init:SubAp_end)-1;
%
...
% structures
moment_inter_chunks_array = zeros(numXX * vx, numYY * vy); %Stitched PowerDoppler moments in each subaperture
moment_inter_chunks_crop_array = zeros(resize_ratio * ac.numX, resize_ratio * ac.numY);
...
stride_x = floor((ac.numX - numXX) / (vx - 1));
stride_y = floor((ac.numY - numYY) / (vy - 1));
offset_x = floor((ac.numX - (stride_x * (vx - 1) + numXX)) / 2);
offset_y = floor((ac.numY - (stride_y * (vy - 1) + numYY)) / 2);
...
for inter_SubAp_idy = inter_SubAp_id_range
    ...
    for inter_SubAp_idx = inter_SubAp_id_range
        ...
        idx_range = offset_x + stride_x * (inter_SubAp_idx - 1) + 1:offset_x + stride_x * (inter_SubAp_idx - 1) + numXX;
        idy_range = offset_y + stride_y * (inter_SubAp_idy - 1) + 1:offset_y + stride_y * (inter_SubAp_idy - 1) + numYY;
        idx_range_ref = offset_x + stride_x * (inter_SubAp_idx_ref - 1) + 1:offset_x + stride_x * (inter_SubAp_idx_ref - 1) + numXX;
        idy_range_ref = offset_y + stride_y * (inter_SubAp_idy_ref - 1) + 1:offset_y + stride_y * (inter_SubAp_idy_ref - 1) + numYY;
        % get the current image chunk
        ...
        moment_inter_chunks_array((inter_SubAp_idy - 1) * numYY + 1:(inter_SubAp_idy) * numYY, (inter_SubAp_idx - 1) * numXX + 1:(inter_SubAp_idx) * numXX) = moment_inter_chunk;
        ...
        if sum(moment_inter_chunk(:)) ~= 0
            ...
            if calibration
                ...
                if strcmp(obj.ref_image, 'central subaperture')
                    moment_chunk_ref = moment_inter_chunks_array((inter_SubAp_idy_ref - 1) * numYY + 1:(inter_SubAp_idy_ref) * numYY, (inter_SubAp_idx_ref - 1) * numXX + 1:(inter_SubAp_idx_ref) * numXX);
                else
                    moment_chunk_ref = G;
                end
        ...
        stride_x = resize_ratio * floor((ac.numX - numXX) / (vx - 1));
        stride_y = resize_ratio * floor((ac.numY - numYY) / (vy - 1));
