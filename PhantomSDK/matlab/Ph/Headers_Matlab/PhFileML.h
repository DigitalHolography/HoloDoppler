...
typedef PVOID CINEHANDLE, *PCINEHANDLE;
...
//-------------------------MATLAB USE START---------------------->>>
//***Declarations from PhInt and PhCon, some are deprecated***
                            ...
                            // fixed point (32.32 seconds)
{
    ...
    UINT32 seconds;       // Seconds from Jan 1 1970, compatible with the C
                            // library routines
...
{
    UINT32 dim;        //square kernel dimension 3,5
    UINT32 shifts;     //right shifts of Coef (8 shifts means divide by 256)
    UINT32 bias;       //bias to add at end
    UINT32 Coef[5*5];  //maximum alocation for a 5x5 filter
}
IMFILTER, *PIMFILTER;
