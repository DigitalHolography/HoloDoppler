...
if use_gpu
    ...
    switch spatial_transformation
        case 'angular spectrum'
            FH = fftshift(fft2(gpuArray(frame_batch))) .* app.kernelAngularSpectrum;
        case 'Fresnel'
            FH = gpuArray((frame_batch) .* app.kernelFresnel);
    end
    ...
    switch spatial_transformation
        case 'angular spectrum'
            FH = fftshift(fft2(frame_batch)) .* app.kernelAngularSpectrum;
        case 'Fresnel'
            FH = (frame_batch) .* app.kernelFresnel;
    end
...
if found
    ...
    if isfolder(fullfile(app.filepath, found_dir, 'pulsewave', 'mask'))
        ...
        maskArtery = imresize(maskArtery, [app.numY, app.numX]) > 0;
        % maskNeighbors = imresize(maskNeighbors,[app.numY,app.numX])>0;
