    ...
    parfor (batch_idx = 1:local_num_batches, parfor_arg)
        ...
        if svdx
            if svd_treshold
                ...
                H = svd_x_filter(H, local_f1, local_fs, Nb_SubAp);
            end
        end
        SH = fft(H, [], 3);
        SH = abs(SH).^2;
        ...
        SH = permute(SH, [2 1 3]);
        M0 = m0(SH, f1, f2, ac.fs, j_win, gaussian_width); % with flatfield of gaussian_width applied
        moment0 = m0(SH, f1, f2, ac.fs, j_win, 0); % no flatfield : raw
        moment1 = m1(SH, f1, f2, ac.fs, j_win, 0);
        moment2 = m1(SH, f1, f2, ac.fs, j_win, 0);
