...
% H_retina = H_retina .* make_ring_mask(numX, numY, 125, 0);
% figure(42);
...
% unit cell size (x,y)
numX_pattern = floor(numX / num_unit_cells_x);
numY_pattern = floor(numY / num_unit_cells_y);
r1_retina = ceil(sqrt(2) * max(numX_pattern, numY_pattern));
% r2_retina = 15;
% mask_blur_retina = 1;
retina_mask_centered_pattern = make_ring_mask(numX_pattern, numY_pattern, r1_retina, r2_retina);
...
n1 = ceil(f1 * numFrames / fs);
n2 = ceil(f2 * numFrames / fs);
...
x_neighborhood = 0;
y_neighborhood = 0;
iris_mask_centered_pattern = make_ring_mask(numX_pattern, numY_pattern, r1_iris, r2_iris);
...
angular_mask_centered = single(make_ring_mask(numX, numY, r1_FH, r2_FH));
angular_mask_centered = gpuArray(single(imgaussfilt(angular_mask_centered, mask_blur_angular)));

% evaluation of iris plane size
numX2 = ceil(numX / x_stride) * (2 * x_neighborhood + 1);
numY2 = ceil(numY / y_stride) * (2 * y_neighborhood + 1);
% memory allocation of iris plane complex matrix
dark_field_H = gpuArray(single(zeros(numX2, numY2, numFrames)) + 1i * (zeros(numX2, numY2, numFrames)));
% construction of propagation kernels
kernel1 = propagation_kernelAngularSpectrum(numX, numY, -z_retina, lambda, x_step, y_step, false);
kernel2 = propagation_kernelAngularSpectrum(numX, numY, z_iris, lambda, x_step, y_step, false);
...
center_x = floor(numX_pattern / 2);
center_y = floor(numY_pattern / 2);
...
for id_y = 1:y_stride:numY_pattern %
    ii_y = ii_y + 1;
    ii_x = 0;
    % Update waitbar and message
    waitbar(id_y / numY_pattern, f, ['compute image ', num2str(round(id_y / numY_pattern * 100), 2), ' percent']);

    for id_x = 1:x_stride:numX_pattern %
        ii_x = ii_x + 1;
        ...
        for pp_y = 1:num_unit_cells_y
            ...
            for pp_x = 1:num_unit_cells_x
                jx_range = (pp_x - 1) * numX_pattern + 1:(pp_x) * numX_pattern;
                jy_range = (pp_y - 1) * numY_pattern + 1:(pp_y) * numY_pattern;
        ...
        %% sourcepoint
        %         retina_mask_sourcepoint = make_ring_mask(numX, numY, id_x, id_y, r1_retina_sourcepoint, r2_retina_sourcepoint);
        ...
        Mx_pattern = numX2 / num_unit_cells_x;
        My_pattern = numY2 / num_unit_cells_y;
        ...
        for pp_y = 1:num_unit_cells_y
            ...
            for pp_x = 1:num_unit_cells_x
                ...
                x2_range = center_x + (pp_x - 1) * numX_pattern - x_neighborhood:center_x + (pp_x - 1) * numX_pattern + x_neighborhood;
                y2_range = center_y + (pp_y - 1) * numY_pattern - y_neighborhood:center_y + (pp_y - 1) * numY_pattern + y_neighborhood;
...
close(f);
% kernel1 = propagation_kernelAngularSpectrum(numX2,numY2, z_retina, lambda, x_step, y_step, false);
% kernel2 = propagation_kernelAngularSpectrum(numX2,numY2, -z_iris , lambda, x_step, y_step, false);
...
% toc
dark_field_H = imresize(dark_field_H, [numX, numY], 'nearest');
end
