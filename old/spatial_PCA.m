...
num_cube_x = 16;
num_cube_y = 16;
numX_cube = floor(size(H, 1)/num_cube_x);
numY_cube = floor(size(H, 2)/num_cube_y);
H_filtered = 1i*zeros(size(H), 'double');

cube = 1i*zeros(numX_cube, numY_cube, size(H, 3), num_cube_x, num_cube_y);

for id_y = 1 : num_cube_y
    for id_x = 1 : num_cube_x
        % cut out the cube of interest
        range_x = (id_x - 1) * numX_cube + 1: id_x * numX_cube;
        range_y = (id_y - 1) * numY_cube + 1: id_y * numY_cube;
...
for id_y = 1 : num_cube_y
    for id_x = 1 : num_cube_x
        ...
        minicube = reshape(minicube, numX_cube * numY_cube, size(H,3));
