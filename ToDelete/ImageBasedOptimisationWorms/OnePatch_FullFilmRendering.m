...
DataFileName = '16_0063_XY_D_512_512_8bit_e';%astig 1 cylindrical lens
...
PathName_I = ['../InterferogramsWorms/' DataFileName '.raw'];
...
delta_x = 0; % for 180810_Leo_OD_60k_acq11
delta_y = 0; % for 180810_Leo_OD_60k_acq11

gw = 35; % width of gaussian for "flat field" correction

ff = dir(PathName_I);
TotalImageNumber = ff.bytes/(Nx*Ny); %259329 for 512x512 format and full acquisition
Nt = TotalImageNumber;
% Nt = 10;
...
for offset = 0:jstep*Nx*Ny:(Nt-jwin)*Nx*Ny  
        fid_I = fopen(PathName_I, 'r');
        fseek(fid_I,offset,'bof'); % 16bit/px
        I2 = fread(fid_I,Nx*Ny*jwin,'uint8=>double','b'); % 16bit/px, 'b'= big endian ordering
        fclose(fid_I);
