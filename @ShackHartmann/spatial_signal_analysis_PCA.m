...
for id_y = 1 : n_subAp_x
    for id_x = 1 : n_subAp_x
        FH((id_y-1)*subAp_Nx+1 : id_y*subAp_Nx, (id_x-1)*subAp_Nx+1 : id_x*subAp_Nx, :) = FH((id_y-1)*subAp_Nx+1 : id_y*subAp_Nx, (id_x-1)*subAp_Nx+1 : id_x*subAp_Nx, :).* V(id_x + (id_y - 1)*n_subAp_x,1); % we select the highest energy trends
    end
end 
