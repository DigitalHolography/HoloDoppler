classdef ImageTypeList < handle % This class is modified dynamically
    ...
    methods
        ...
        function construct_image(obj, FH, wavelength, ac, gaussian_width, use_gpu, svd, ...
            ...
            if (SubAp_PCA.Value)
                FH = subaperture_PCA(FH, SubAp_PCA, ac, svd, time_transform.f1, time_transform.f2, gaussian_width);
            end
            ...
            if obj.dark_field_image.is_selected
                obj.dark_field_image.image = log(flat_field_correction(moment0(SH, f1, f2, ac.fs, batch_stride, gaussian_width), gaussian_width));
            end
            ...
            if obj.phase_variation.is_selected
                ...
                % C = permute(C, [2 1 3]);
                obj.phase_variation.image = moment0(SH, f1, f2, ac.fs, batch_stride, gaussian_width);
            end

            if obj.power_Doppler.is_selected % Power Doppler has been chosen
                [img, sqrt_img] = moment0(SH, f1, f2, ac.fs, batch_stride, gaussian_width);
            ...
            if obj.power_1_Doppler.is_selected % Power 1 Doppler has been chosen
                img = moment1(SH, f1, f2, ac.fs, batch_stride, gaussian_width);
                [img0, ~] = moment0(SH, f1, f2, ac.fs, batch_stride, 0);
                img = img ./ mean(img0(:));
                obj.power_1_Doppler.image = img;
            end

            if obj.power_2_Doppler.is_selected % Power 2 Doppler has been chosen
                img2 = moment2(SH, f1, f2, ac.fs, batch_stride, 0);
                [img0, ~] = moment0(SH, f1, f2, ac.fs, batch_stride, 0);
                obj.power_2_Doppler.image = sqrt(img2 ./ mean(img0(:)));
            end

            if obj.color_Doppler.is_selected % Color Doppler has been chosen
                [freq_low, freq_high] = composite(SH, color_f1, color_f2, color_f3, ac.fs, batch_stride, gaussian_width);
            ...
            if obj.directional_Doppler.is_selected % Directional Doppler has been chosen
                [M0_pos, M0_neg] = directional(SH, f1, f2, ac.fs, batch_stride, gaussian_width);
            ...
            if obj.M0sM1r.is_selected % M1sM0r has been chosen
                img = fmean(SH, f1, f2, ac.fs, batch_stride, gaussian_width);
                obj.M0sM1r.image = img;
            end

            if obj.velocity_estimate.is_selected % Velocity Estimate has been chosen
                obj.velocity_estimate.image = construct_velocity_video(SH, f1, f2, ac.fs, batch_stride, gaussian_width, wavelength);
            end
            ...
            if obj.choroid.is_selected % Power 1 Doppler has been chosen
                ...
                for freqIdx = 1:num_F
                    img = moment0(SH, frequencies(freqIdx), frequencies(end), ac.fs, batch_stride, gaussian_width);
                    img = img / (sum(img .* circleMask, [1 2]) / nnz(circleMask));

                    obj.choroid.parameters.intervals_0(:, :, :, freqIdx) = img;

                    img = moment1(SH, frequencies(freqIdx), frequencies(end), ac.fs, batch_stride, gaussian_width);
                    img = img / (sum(img .* circleMask, [1 2]) / nnz(circleMask));
