...
def _process_batch_worker(
    batch_index, file_path, parameters, first_frame, batch_stride, batch_size
):
    """
...
def preview(file_path, parameters, save_debug=True):
    ...
    return preview_image_from_results(res)
...
def process(file_path, parameters, progress_callback=None):
    ...
    # ------------------------------------------------------------------
    n_workers = parameters.get(
        "num_workers", min(int(cpu_count() // 2), num_batch)
    )  #  # use all available cores, capped by number of batches
    worker_func = partial(
    ...
    with Pool(processes=n_workers) as pool:
        ...
        batch_results = []
        results = tqdm(pool.imap(worker_func, range(num_batch)), total=num_batch)
    ...
    if parameters.get("image_registration", False) and "M0ff" in output:
        ...
        reg_keys = [
            ...
            if k in ["M0ff", "M0", "M1", "M2"] or k.startswith("band_")
        ]
        ...
        for i in range(num_batch):
            ...
            shifts_x.append(sx)
            for k in reg_keys:
                output[k][i] = apply_register_images_shifts(
                    np, np.fft, output[k][i], sy, sx
                )

        output["registration"] = np.stack(
            [np.array(shifts_y), np.array(shifts_x)], axis=1
        )
    ...
    if parameters.get("square", False):
        ...
        for key, value in output.items():
            if value.ndim >= 3:
                size = max(value.shape[-2:])
                squared[key] = resize_slicewise(
                    value,
                    size,
                    size,
                    axes=(-2, -1),
