...
def print_timings_summary():
    ...
    print("-" * 78)
    sorted_funcs = sorted(
        _func_timings.items(), key=lambda x: x[1]["cum_time"], reverse=True
    )
    for fname, data in sorted_funcs:
        ratio = data["cum_time"] / total if total > 0 else 0
        print(
            f"{fname:<45} | {data['calls']:>5} | {data['cum_time']:>10.4f} | {ratio:>6.2%}"
        )
    print(f"{'TOTAL':<45} | {'':>5} | {total:>10.4f} | 100.00%")
    print(
        f"{'CUMULATED':<45} | {'':>5} | {cumulated:>10.4f} | {(cumulated / total if total > 0 else 0) :>6.2%}"
    )
    print("-" * 78)


construct_subapertures_fresnel = track_time("construct_subapertures_fresnel")(
    construct_subapertures_fresnel
)
construct_subapertures_angular = track_time("construct_subapertures_angular")(
    construct_subapertures_angular
)
calculate_displacements_graph_laplacian = track_time(
    "calculate_displacements_graph_laplacian"
)(calculate_displacements_graph_laplacian)
...
fit_zernike_angular_spectrum = track_time("fit_zernike_angular_spectrum")(
    fit_zernike_angular_spectrum
)
southwell_phase_integration = track_time("southwell_phase_integration")(
    southwell_phase_integration
)
pad_array_centrally = track_time("pad_array_centrally")(pad_array_centrally)
fresnel_transform_with_phase = track_time("fresnel_transform_with_phase")(
    fresnel_transform_with_phase
)
angular_spectrum_transform_with_phase = track_time(
    "angular_spectrum_transform_with_phase"
)(angular_spectrum_transform_with_phase)
fresnel_transform = track_time("fresnel_transform")(fresnel_transform)
angular_spectrum_transform = track_time("angular_spectrum_transform")(
    angular_spectrum_transform
)
...
frequency_symmetric_filtering = track_time("frequency_symmetric_filtering")(
    frequency_symmetric_filtering
)
...
DebugPlotterManager.plot_all = track_time("DebugPlotterManager.debug_plot_all")(
    DebugPlotterManager.plot_all
)
CineFileReader.read_frames = track_time("CineFileReader.read_frames")(
    CineFileReader.read_frames
)
HoloFileReader.read_frames = track_time("HoloFileReader.read_frames")(
    HoloFileReader.read_frames
)
    ...
    if parameters.get("shack_hartmann_graph_laplacian", False):
        ...
        if imref == "ref_from_registration":
            imref = (
                ...
                else resize_slicewise(registration_ref, Ny, Nx, xp=xp, fft=fft)
            )
...
def _process_sub_batch(bm, parameters, frames_sub, phase_term, compute_debug):
    ...
    # SVD filtering
    if compute_debug:
        holograms_f, removedU, holograms_fbar, eigenvalues, _, _, dc = svd_filter(
            xp,
            holograms,
...
def render_moments(bm, parameters, frames=None, registration_ref=None):
    ...
    if parameters.get("shack_hartmann", False):
        phase_term, debug_sh = _process_shack_hartmann(
            bm, parameters, frames, registration_ref
        )
        if compute_debug:
            res.update(debug_sh)
    ...
    for it in range(main_acc.batch_size):
        ...
        batch = _process_sub_batch(
            bm, parameters, frames_sub, phase_term, compute_debug
        )
    ...
    if compute_debug and "psd" in res:
        accu["psd_map_avg"] = res.pop(
            "psd"
        )  # transfer to the accu dict where results are accumulated through all iterations

    # --- Image registration ---
    if parameters.get("image_registration", False) and registration_ref is not None:
        M0_ff = res[
            "M0ff"
        ]  # gaussian_flatfield(res["M0"], parameters.get("registration_flatfield_gw", 1.0), bm.gaussian_filter)
        if compute_debug:
        ...
        reg = register_trs(
            ...
            estimate_similarity=(
                parameters.get("image_registration_type")
                == "translation_rotation_scale"
            ),
            gaussian_sigma=parameters.get("registration_gaussian_sigma"),
            integer_translation=parameters.get(
                "registration_integer_translation", False
            ),
        )

        if parameters.get("apply_registration", False):
            res["M0"] = apply_registration(
                xp,
                bm.fft,
                bm.ndi,
                res["M0"],
            ...
            if "M0svdbar" in res:
                res["M0svdbar"] = apply_registration(
                    xp,
                    bm.fft,
                    bm.ndi,
                    res["M0svdbar"],
            ...
            if "psd_map_avg" in accu:
                accu["psd_map_avg"] = apply_registration3D(
                    xp,
                    bm.fft,
                    bm.ndi,
                    accu["psd_map_avg"],
...
def preview_process_moments(file_path, parameters, save_debug=True):
    ...
    if (
        compute_debug
        and parameters.get("shack_hartmann")
        and parameters.get("shack_hartmann_zernike_fit")
    ):
...
def process_moments(
    ...
    if parameters.get("image_registration"):
        frames_reg = file_reader.read_frames(
            ...
            batch_size=parameters.get("batch_size_registration", batch_size),
        )
            ...
            for key in debug_results[0].keys():
                try:
                    vid_debug[key] = np.stack(
                        [bm.to_numpy(debug_results[k][key]) for k in range(num_batch)],
                        axis=0,
                    )
                except Exception as e:
                    # import traceback
...
def _process_cpu_streaming(
    ...
    for i in tqdm(range(num_batch)):
        ...
        res, accu = render_moments(
            bm, parameters, frames=frames, registration_ref=M0_reg
        )
    ...
    try:
        ...
        for i, frames in enumerate(
            ...
            if d_current is not None:
                ...
                with compute_stream:
                    res, accu = render_moments(
                        bm, parameters, frames=d_current, registration_ref=M0_reg
                    )
