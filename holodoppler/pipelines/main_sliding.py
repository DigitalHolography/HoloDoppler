...
def preview(file_path, parameters):
    ...
    for n in range(time_slide_repetition):
        frames = file_reader.read_frames(
            first_frame=first_frame + n * time_slide_delay, batch_size=time_window
        )
        ...
        # calc on gpu
        _process_batch(
            parameters, frames=frames, phase_term=phase_term, output_dict=res
        )
...
def process(file_path, parameters):
    ...
    if time_stride >= (end_frame - first_frame):
        num_batch = (
            1
            if time_window + time_slide_delay * time_slide_repetition
            <= (end_frame - first_frame)
            else 0
        )
    else:
        num_batch = int(
            (end_frame - first_frame - time_slide_delay * time_slide_repetition)
            / time_stride
        )
    ...
    for i in tqdm(range(num_batch)):
        ...
        for n in range(time_slide_repetition):
            ...
            with h2d_stream:
                ...
                frames = file_reader.read_frames(
                    first_frame=first_frame + i * time_stride + n * time_slide_delay,
