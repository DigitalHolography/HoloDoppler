...
from holodoppler.filtering import svd_filter, frequency_symmetric_filtering, fourier_time_transform, corner_compensation, pca_time_transform
...
def accumulate_on_frames(file_reader, n_accu, first_frame, batch_size, batch_stride, func):
    U = None
...
def preview(file_path, parameters):
    ...
    if parameters.get("shack_hartmann",False):
        U_tot = accumulate_on_frames(file_reader, n_accu, first_frame, batch_size, batch_stride, lambda frames : _process_shack_hartmann_U(parameters, frames))
    res = {}
    ...
    Projection_tot = accumulate_on_frames(file_reader, n_accu, first_frame, batch_size, batch_stride, lambda frames : _process_batch(parameters, frames = frames, phase_term = phase_term))
    ...
    save_preview_images(res_np, _get_default_output_path(file_reader.file_path) / "preview"/ "PCA_ACCU", square=parameters["square"])
...
def process(file_path, parameters):
    ...
    for i in tqdm(range(num_batch)):
        ...
        Projection_tot = accumulate_on_frames(file_reader, n_accu, first_frame + i * batch_stride * n_accu, batch_size, batch_stride, lambda frames : _process_batch(parameters, frames = frames, phase_term = phase_term))
        Projection_tot_ff = gaussian_flatfield(Projection_tot, parameters.get("registration_flatfield_gw", 35.0), gaussian_filter)
