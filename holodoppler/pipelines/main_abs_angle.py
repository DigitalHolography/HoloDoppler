...
def _process_batch(parameters, frames, phase_term = None, output_dict = None):
    ...
    if parameters.get("temporal_transformation") == "FourierTransform":
        ...
        spectrum_f_abs = fourier_time_transform(xp, fft, xp.abs(holograms_f))
    else:
    ...
    psd_abs = xp.abs(spectrum_f_abs) ** 2
    ...
    # Moments of phase spectrum
    output_dict["M0_phase"] = moment(xp, psd_angle[idxs], freqs, 0)
    output_dict["M1_phase"] = moment(xp, psd_angle[idxs], freqs, 1)

    output_dict["spectrum_line_phase"] = xp.mean(psd_angle, axis=(-2,-1))

    # Moments of amplitude spectrum
    output_dict["M0_abs"] = moment(xp, psd_abs[idxs], freqs, 0)
    output_dict["M1_abs"] = moment(xp, psd_abs[idxs], freqs, 1)

    output_dict["spectrum_line_abs"] = xp.mean(psd_abs, axis=(-2,-1))
